startReactor = {
  ...
  play(index) {
      ...
      if (startReactor.isTheRightCombination(startReactor.playerCombination.length)) {
          ...
          document.getElementById("title").textContent = "VOCÊ ERA UM IMPOSTOR"
          setTimeout(() => {
...
/* startReactor = {
  ...
  interface: {
    ...
    turnAllLedsOff() {
      ...
      for (var x = 0; x < computerLedPanel.children.lenght; i++) {
        computerLedPanel.children[x].remove("ledOn");
        playerLedPanel.children[x].remove("ledOn");
      }
    },
    ...
    playItem(index, combinationPosition, location = "computer") {
      ...
      memPanel.classList.add("memoryActive");
      startReactor.interface.turnLedOn(combinationPosition, leds);
      startReactor.audio.combination[index].play().then(() => {
        setTimeout(() => {
          memPanel.classList.remove("memoryActive");
        }, 200);
      });
    ...
    endGame(type = "fail") {
      const memPanel = startReactor.interface.memoryPanel;
      const ledPanel = startReactor.interface.ledPanel;
      const audio =
        type == "complete"
          ? startReactor.audio.complete
          : startReactor.audio.fail;
      ...
      audio.play().then(() => {
        for (var x = 0; x < memPanel.children.lenght; x++) {
          if (memPanel.children[x].tagName == "DIV")
            memPanel.children[x].classList.add(typeClasses[0]);
        }
        for (var x = 0; x < ledPanel.children.lenght; x++) {
          if (ledPanel.children[x].tagName == "DIV")
            ledPanel.children[x].classList.add(typeClasses[1]);
        }
        setTimeout(() => {
          for (var x = 0; x < memPanel.children.lenght; x++) {
            if (memPanel.children[x].tagName == "DIV")
              memPanel.children[x].classList.remove(typeClasses[0]);
          }
          for (var x = 0; x < ledPanel.children.lenght; x++) {
            if (ledPanel.children[x].tagName == "DIV")
              ledPanel.children[x].classList.remove(typeClasses[1]);
          }
        }, 900);
      });
    },
    ...
    enableButtons() {
      ...
      for (var x = 0; x < playerMemory.children.lenght; x++) {
        if (playerMemory.children[x].tagName == "DIV")
          playerMemory.children[x].classList.add("playerMemoryActive");
      }
    },
    ...
    disableButtons() {
      ...
      for (var x = 0; x < playerMemory.children.lenght; x++) {
        if (playerMemory.children[x].tagName == "DIV")
          playerMemory.children[x].classList.remove("playerMemoryActive");
      }
    },
    async load() {
      return new Promise((resolve) => {
    ...
    play(index) {
      startReactor.interface.playItem(
        index,
        startReactor.playCombination.lenght,
        "player"
      );
      startReactor.playCombination.push(index);

      if (
        startReactor.isTheRightCombination(startReactor.combinationMaxPosition)
      ) {
        if (
          startReactor.playCombination.lenght ==
        ...
        startReactor.interface.endGame("fail");
        document.getElementById("title").textContent = "VOCÊ É UM IMPOSTOR";
        setTimeout(() => {
          document.getElementById("title").textContent = "START REACTOR";
          startReactor.start();
        }, 1200);
